...
{   
    ...
    public function completed(){
        if(Session::get('role') == 'admin'){
            ...
            $id=Session::get('id');
            $posts = DB::select("select posts.id,posts.purpose,posts.meetingdate,posts.starttime,posts.endtime,posts.total,posts.postingdate,categories.name category,rooms.name room,users.name postuser,posts.status,posts.meetingtype from users,posts,categories,rooms where posts.status='booked' and users.id=posts.postuser_id and categories.id=posts.category_id and rooms.id=posts.room_id and posts.meetingdate < 
            DATE(NOW()) and posts.postuser_id='$id' order by posts.meetingdate asc");
        }
        $i=1;
        foreach($posts as $post){
            echo 
    ...
    public function today(){
        if(Session::get('role') == 'admin'){
            $posts = DB::select("select posts.id,posts.purpose,posts.meetingdate,posts.starttime,posts.endtime,posts.total,posts.postingdate,categories.name category,rooms.name room,users.name postuser,posts.status,posts.meetingtype from users,posts,categories,rooms where posts.status='booked' and users.id=posts.postuser_id and categories.id=posts.category_id and rooms.id=posts.room_id and posts.meetingdate=DATE(NOW()) order by posts.meetingdate asc");
        }else{
            $id=Session::get('id');
            $posts = DB::select("select posts.id,posts.purpose,posts.meetingdate,posts.starttime,posts.endtime,posts.total,posts.postingdate,categories.name category,rooms.name room,users.name postuser,posts.status,posts.meetingtype from users,posts,categories,rooms where posts.status='booked' and users.id=posts.postuser_id and categories.id=posts.category_id and rooms.id=posts.room_id and posts.meetingdate=DATE(NOW()) order by posts.meetingdate asc");
        }
