...
class reportController extends Controller
{
    ...
    public function search(Request $r){
        ...
        $datefrom=$r->datefrom;
        $dateto=$r->dateto;
        $query="select posts.id,posts.purpose,posts.meetingdate meetingdate,posts.starttime,posts.endtime,posts.total,posts.postingdate,posts.category category,posts.room room,users.name postuser,posts.status,posts.meetingtype from users,posts,categories,rooms where users.id=posts.postuser_id";
        if($status != ""){
            if($status == 'booked'){
